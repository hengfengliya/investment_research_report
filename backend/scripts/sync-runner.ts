      ...
      limit(async () => {
        try {
          ...
          const authors = normalizeAuthors(
            record.author ?? record.researcher,
          );
          ...
          const reportData = {
            ...
            sourceUrl,
            stockCode:
              (record.stockCode as string | undefined) ??
              detail.stockCode ??
              null,
            stockName:
              (record.stockName as string | undefined) ??
              detail.stockName ??
              null,
            industry:
              (record.industryName as string | undefined) ??
              detail.industryName ??
              null,
            rating:
              ...
              null,
            ratingChange:
              record.ratingChange !== undefined
                ? String(record.ratingChange)
                : null,
            targetPrice:
              toNumber(record.indvAimPriceT ?? record.indvAimPriceL) ?? null,
            changePercent: toNumber(record.changePercent),
            topicTags: deduplicateTags(detail.topicTags),
            impactLevel:
              category === "strategy" || category === "macro"
                ? detail.impactLevel
                : null,
...
export const runSyncOnce = async (): Promise<SyncSummary> => {
  ...
  return {
    totalFetched: categories.reduce((sum, item) => sum + item.fetched, 0),
    totalInserted: categories.reduce((sum, item) => sum + item.inserted, 0),
    totalUpdated: categories.reduce((sum, item) => sum + item.updated, 0),
    totalErrors: categories.reduce((sum, item) => sum + item.errors, 0),
    categories,
  };
};
